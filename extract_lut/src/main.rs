...
fn dump_lut_spine<T: LUTConstantType>(name: &str, public: bool, elements: &Vec<ConstantRef>) {
    println!("#[allow(non_upper_case_globals)]");
    println!(
        "{priv}const {name}: [* const {type}; {len}] = [",
        priv = if public { "pub " } else { "" },
        name = name,
        type = T::TYPE_NAME,
        len = elements.len(),
        ...
        } = var.initializer.unwrap().deref()
        {
            match element_type.deref() {
                ...
                Type::PointerType { pointee_type, .. } => match pointee_type.deref() {
                    Type::FPType(FPType::Single) => {
                        dump_lut_spine::<f32>(&name, public, elements);
                    }
